...
public class ARCamera : MonoBehaviour
{
    ...
    private void SetSmoothTran()
    {
        ...
        if (arPose.Count > 3)
        {
            Debug.Log("ARPOS : " + arPose[0].Pos + " : " + arPose[1].Pos + " : " + arPose[2].Pos);
        }
        if (transform.localPosition != Vector3.zero)
        {
            
            //Debug.Log("pos : " + pos + " rot : " + rot+ " count : " + arPose.Count);
            //Debug.Log("ARpos : " + transform.localPosition + " ARrot : " + transform.localRotation);
